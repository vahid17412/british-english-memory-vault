...
// Icon Components mapped explicitly to bypass raw string rendering issues
...
export const AppShell = React.memo(function AppShell({ children, currentRoute = 'home', onNavigate }: AppShellProps) {
  ...
  // Inline styling handling for mobile PWA safe areas without layout breaking dependencies
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen bg-gray-50 dark:bg-gray-950 font-sans">
      {/* Desktop Sidebar */}
      <nav className="hidden md:flex flex-col w-64 bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-800 h-screen sticky top-0 p-4">
            ...
            return (
              <li key={item.id}>
                <button
                  ...
                  className={`w-full flex items-center gap-4 px-4 py-3 rounded-xl transition-colors text-left focus:outline-none focus:ring-2 focus:ring-blue-500 ${
                    isActive 
      ...
      {/* Main Content Area */}
      <div className="flex-1 flex flex-col pb-20 md:pb-0 overflow-y-auto">
        {children}
      </div>

      {/* Mobile Bottom Navigation with PWA safe areas protection */}
      <nav 
        style={safeAreaBottomStyle}
        className="md:hidden fixed bottom-0 left-0 right-0 bg-white/80 dark:bg-gray-900/80 backdrop-blur-md border-t border-t-gray-200 dark:border-t-gray-800 z-50 px-2 pt-1"
      >
