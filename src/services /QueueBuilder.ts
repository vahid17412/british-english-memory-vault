...
export class QueueBuilder {
  ...
  async buildDailyQueue(config: QueueConfig): Promise<readonly Card[]> {
    ...
    const dueReviews = allCards
      ...
      .sort((a, b) => calculateScore(b) - calculateScore(a)) // Sort by Priority Score descending
      .slice(0, config.reviewCardsCap);

    // Strict validation for New Cards
    ...
    // Return Deep Frozen array to prevent UI mutations
    return this.deepFreeze([...dueReviews, ...verifiedNewCards]);
  }

  private deepFreeze<T>(obj: T): T {
    Object.freeze(obj);
    if (Array.isArray(obj)) {
      obj.forEach(item => Object.freeze(item));
    }
    return obj;
  }
}
